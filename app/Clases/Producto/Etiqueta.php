...
class Etiqueta {
	...
	public function getLink() {
		try {
			...
			if(!strpos($xLink, '?filtros=') && !strpos($xLink, '&filtros=') ){
				$xLink .= '?filtros=';
				$xLink .= Sawubona::convertToUrl($vNombres[0]);
			}elseif(strpos($xLink, '&filtros=') === true)
            {
            	if( !strpos($xLink, Sawubona::convertToUrl($vNombres[0])) ){
					$xLink .= '_' . Sawubona::convertToUrl($vNombres[0]);
				}else{
					$xLink .= '&filtros=';
					$xLink .= Sawubona::convertToUrl($vNombres[0]);
				}
			}
			elseif( !strpos($xLink, Sawubona::convertToUrl($vNombres[0])) ){
				$xLink .= '_' . Sawubona::convertToUrl($vNombres[0]);
			}
	...
	protected function setEtiquetaHija($oEtiquetaAPI = null, $oEtiquetaPadre = null) {
		...
		try {
			$oEtiqueta = new Etiqueta($oEtiquetaAPI->id, $oEtiquetaAPI->nombre);
			return $oEtiqueta;
