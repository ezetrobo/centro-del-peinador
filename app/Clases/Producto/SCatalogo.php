...
class SCatalogo {
	...
	private function setCatalogo($vCatalogosAPI = null) {
		...
		try {
			if (is_array($vCatalogosAPI)):
				$this->etiquetas = isset($vCatalogosAPI[0]->etiquetas) ? Etiqueta::cast($vCatalogosAPI[0]->etiquetas) : array();
				$this->categorias = isset($vCatalogosAPI[0]->categorias) ? Categoria::cast($vCatalogosAPI[0]->categorias, true) : array();
				$this->productos = isset($vCatalogosAPI[0]->productos) ? Producto::cast($vCatalogosAPI[0]->productos) : array();
			endif;
		} catch (Exception $e) {
			Log::error($e);
		}
	...
	public function getProductosByID($xIdCatalogo = 0, $xIdCategoria = 0, $xPrioridad = null, $xOffSet = 0, $xLimit = 6, $CampoValor = 'combo', $valorCampo = 'true') {
		try
		{
			$xUrl = config('main.URL_API')
			. "Catalogo?"
			...
			. "&xCampoOrden="
			. "&xOrden="
			. "&xCampoValor=" . $CampoValor
			. "&xValorCampo=" . $valorCampo;
			
			$this->getCatalogo($xUrl);

		} catch (Exception $e) {Sawubona::writeLog($e);}
