...
class Sawubona {
	...
	public static function getEstadoMP($xCodigo){
		try {
			$fileJson = json_decode(file_get_contents(__DIR__ ."/../runtime/storage/mensajesMercadoPago.json"))[0];
			$mensajes = null;

			if(!empty($fileJson->$xCodigo)):
				$mensajes = $fileJson->$xCodigo."<br><p class='codigo-error'> Código : ".$xCodigo."</p>";
			endif;
